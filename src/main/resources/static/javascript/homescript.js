...
$(function() {
    ...
    $("#exit").click(function() { //if user clicks on exit
        var exit = confirm("Are you sure you want to end the session?"); //and then ok on the confirmation box
        if(exit==true){
            window.location = 'home.php?logout=true'; //if window location is home.php, set logout to true
        }
    });
    //On hover events
    $('.menu-icon')
...
} setInterval (loadDBLog, 10000);	//call the loadLog funtion every 1500 ms